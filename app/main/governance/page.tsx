...
export default function GovernancePage() {
  ...
  const govDeployed = isContractDeployed(MAINNET_JESTERMOGGER_ADDRESS);
        ...
        {!isConnected ? (
          ...
          <div className="beta-card">
            ...
            <p className="beta-card-desc">
              On-chain governance goes live with the mainnet protocol deploy. This
              panel will activate automatically once the Jestermogger contract address
          ...
          <>
            ...
            {view === 'veto-council' && MAINNET_NFT_VETO_COUNCIL_ADDRESS && (
...
function GovernanceStats({ address }: { address: `0x${string}` }) {
  const { data: proposalCount } = useReadContract({
    address: MAINNET_JESTERMOGGER_ADDRESS,
  ...
  const { data: canPropose } = useReadContract({
    address: MAINNET_FRAMEMOGGER_ADDRESS,
    abi: FRAMEMOGGER_ABI,
...
function CreateProposal({ address }: { address: `0x${string}` }) {
  ...
  const buildCall = (): string | {
    ...
    if (template === 'set-alloc') {
      ...
      return {
        target: MAINNET_JESTERGOONER_ADDRESS,
        value: 0n,
        calldata: data,
        description: desc,
      };
    ...
    if (template === 'signal-burn-price') {
      ...
      return {
        target: MAINNET_FRAMEMOGGER_ADDRESS,
        value: 0n,
        calldata: '0x',
        description: desc,
      };
        ...
        <div className="beta-info-box" style={{ marginBottom: '1rem' }}>
          ...
          <div className="beta-input-group">
            ...
            >
              ...
              <option value={MAINNET_MEGACHAD_ADDRESS}>MEGACHAD token</option>
              <option value={MAINNET_MEGAGOONER_ADDRESS}>MEGAGOONER token</option>
              <option value={MAINNET_FRAMEMOGGER_ADDRESS}>Framemogger</option>
              <option value={MAINNET_JESTERGOONER_ADDRESS}>JesterGoonerV3</option>
              <option value={MAINNET_JESTERMOGGER_ADDRESS}>Jestermogger (governance)</option>
            </select>
          </div>
        </div>
      )}
        ...
        <div className="beta-info-box" style={{ marginTop: '0.5rem' }}>
          ...
          <ul style={{ margin: 0, paddingLeft: '1.1rem', fontSize: '0.78rem', listStyle: 'none' }}>
            <li style={{ marginBottom: '0.4rem' }}>
              ...
              <code
                onClick={() => setTargetAddr(MAINNET_MEGACHAD_ADDRESS)}
                style={{ cursor: 'pointer', wordBreak: 'break-all' }}
              >{MAINNET_MEGACHAD_ADDRESS}</code>
            </li>
            <li style={{ marginBottom: '0.4rem' }}>
              <div><strong>MEGAGOONER token</strong> — governance token changes</div>
              <code
                onClick={() => setTargetAddr(MAINNET_MEGAGOONER_ADDRESS)}
                style={{ cursor: 'pointer', wordBreak: 'break-all' }}
              >{MAINNET_MEGAGOONER_ADDRESS}</code>
            </li>
            <li style={{ marginBottom: '0.4rem' }}>
              <div><strong>Framemogger</strong> — parameter changes to the burn mechanism</div>
              <code
                onClick={() => setTargetAddr(MAINNET_FRAMEMOGGER_ADDRESS)}
                style={{ cursor: 'pointer', wordBreak: 'break-all' }}
              >{MAINNET_FRAMEMOGGER_ADDRESS}</code>
            </li>
            <li style={{ marginBottom: '0.4rem' }}>
              <div><strong>JesterGoonerV3</strong> — staking parameter changes (e.g. <code>setAllocPoint</code>)</div>
              <code
                onClick={() => setTargetAddr(MAINNET_JESTERGOONER_ADDRESS)}
                style={{ cursor: 'pointer', wordBreak: 'break-all' }}
              >{MAINNET_JESTERGOONER_ADDRESS}</code>
            </li>
...
function VetoCouncilSection({ address }: { address: `0x${string}` }) {
  ...
  const { data: council, refetch: refetchCouncil } = useReadContract({
    address: MAINNET_NFT_VETO_COUNCIL_ADDRESS,
